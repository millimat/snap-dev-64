...
bool TNEANet::GetEulerPath(TInt64V& Result) {
  Result = TInt64V();
  int64 StartNId;
  if (!IsEulerian(&StartNId)) { return false; }
  
  // Use Hierholzer's algorithm to generate Euler path.
  // 1. For each node, record which of its incident edges are self edges or outbound edges.
  ...
  THash<TInt64, TVec<TInt64L> > Paths; // Node id -> all paths that start at this node
 
  // 2. Create a set of edge-disjoint paths that span all edges of the graph.
  ...
  while (!OutboundEdges.Empty()) { 
    ...
    while (OutboundEdges.IsKey(currnode.GetId())) {
      ...
      if (!currself.Empty()) { // First time visit; get self edges out of the way
        for (int i = 0; i < currself.Len(); i++) { currpath.AddBack(currnode.GetOutEId(currself[i])); }
        currself.Clr();
      }

      // Visit the first available edge out of this node.
      TInt64 outrelid = curroutbound.BegI().GetKey();
      TInt outeid = currnode.GetOutEId(outrelid);
      TEdgeI outedge = GetEI(outeid);
      currpath.AddBack(outeid);
  ...
  // 3. Traverse the first/"main" path. Each time we encounter a node with a different cycle
  // starting at it, wire it into the main path, then continue.
  TVec<TInt64L>& PathsFromStart = Paths.GetDat(StartNId);
  PathsFromStart.Swap(0, PathsFromStart.Len()-1); // so main path can be removed efficiently
  TInt64L MainPath = PathsFromStart.Last();
  PathsFromStart.DelLast();

  TLstNd<TInt64>* Curr = MainPath.First();
  TInt64 CurrNId = StartNId;
  for (TLstNd<TInt64>* Curr = MainPath.First(); Curr != NULL; Curr = Curr->NextNd) {
    if (Paths.IsKey(CurrNId)) { // More branches found; add them to current
      TVec<TInt64L>& PathsStartingHere = Paths.GetDat(CurrNId);
      while (!PathsStartingHere.Empty()) {
        MainPath.InsLst(Curr, PathsStartingHere.Last());
        PathsStartingHere.DelLast();
      }
      Paths.DelKey(CurrNId);
    }
    Result.Add(Curr->GetVal());
    CurrNId = GetEI(Curr->GetVal()).GetDstNId();
  }
